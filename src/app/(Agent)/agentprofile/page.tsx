...
const Page = () => {
  ...
  const socialLinks = [
    ...
    { key: 'x', url: socials.x, Icon: FaXTwitter },
        ...
        <div className="w-full">
          ...
          <div className="bg-[#303030] rounded-2xl p-2 md:p-4 lg:p-6 flex items-center justify-between mb-2">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-white text-xl font-semibold">{user?.name || 'N/A'}</h1>
                <p className="text-gray-400 text-sm capitalize">{user?.role || 'N/A'}</p>
              </div>
            </div>
            <Link href="/editagentprofile">
          ...
          <div className="bg-[#303030] px-6 py-4 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                  <span className="text-gray-400 text-sm min-w-[120px]">Role</span>
                  <span className="text-white text-sm capitalize">{user?.role || 'N/A'}</span>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <span className="text-white text-sm">
                    {user?.avgRating ?? 0} ({user?.totalRating ?? 0} ratings)
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <span className="text-white text-sm">
                    {profile?.service?.serviceName || 'N/A'}
                  </span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-gray-400 text-sm min-w-[120px]">Service Info</span>
                  <span className="text-white text-sm">
                    {profile?.service?.description || 'N/A'}
                  </span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-gray-400 text-sm min-w-[120px]">Experience</span>
                  <span className="text-white text-sm">{profile?.experiences || 'N/A'}</span>
                </div>

                <div className="flex items-start gap-3">
                  <span className="text-gray-400 text-sm min-w-[120px]">Expertise</span>
                  <div className="flex gap-2 flex-wrap">
                    {profile?.areaOfExpertise?.length ? (
                      profile.areaOfExpertise.map((skill:any, idx:number) => (
                        <span
                          key={idx}

...
import { useGetClubRecommendationsQuery, useGetSingleClubQuery, useUpdateClubMutation } from "@/redux/features/club/club";
...
const Page = () => {
  ...
  return (
    <div className=" md:p-6">
      ...
      <div className="lg:flex items-start space-y-3 lg:space-x-5">
        <div className="w-full lg:w-[24%] bg-[#303030] p-2 mt-4 rounded-lg">
          {isClubLoading && (
            <div className="h-64 rounded-xl bg-[#1a1a1a] animate-pulse" />
          )}

          {!isClubLoading && isClubError && (
            <p className="text-red-500 text-sm">
              Couldn&apos;t load post details. Try again.
            </p>
          )}

          {!isClubLoading && !isClubError && club && (
            <div className="shrink-0 p-1 flex flex-col">
              <div className="flex flex-col items-center text-center mb-2">
                <h1 className="text-base text-white font-semibold tracking-tight">
                  {club.positionTitle}
                </h1>
              </div>
              ...
              <div className="space-y-5 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white/80 font-medium">
                    {club.dateLine ? new Date(club.dateLine).toLocaleDateString() : "—"}
                  </span>
                </div>
        ...
        <div className="w-full lg:w-[74%] space-y-3 ">
          {recommendations.map((rec) => {
            ...
            const isUpdatingRow = updatingId === rec._id;
                  ...
                  {status === "pending" ? (
                    <>
                      <button
                        ...
                        className="rounded-lg border border-white/15 px-4 py-1.5 text-sm text-white/80 hover:bg-white/5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Reject
                      </button>
                      <button
                        ...
                        disabled={isUpdatingRow}
                        className="rounded-lg bg-red-600 px-4 py-1.5 text-sm text-white hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Accept
                      </button>
                    </>
...
//     const { data:clubData, } = useGetSingleClubQuery(id as string);
//     const club = clubData?.data;
//    console.log(club)
...
//         </p>
//       )}
//       <div className="flex items-start space-y-3 md:space-x-2">
...
//       </div>
//       </div>
//       <div className="w-full md:w-[74%] space-y-3">
//         {recommendations.map((rec) => {
//           const status = localStatus[rec._id] ?? rec.status;

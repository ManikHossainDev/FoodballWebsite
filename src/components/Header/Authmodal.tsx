...
// Small shared helper: pulls a readable error message out of an RTK Query
// result object, whatever shape the backend/error happens to be in.
const getErrorMessage = (res: any, fallback: string) => {
  if (res?.data?.message) return res.data.message;
  if (res?.error) {
    if ("data" in res.error) {
      return (res.error.data as any)?.message || fallback;
    }
    if ("error" in res.error && typeof res.error.error === "string") {
      // RTKQ FetchBaseQueryError network/parsing failure shape
  ...
  const handleRoleSubmit = async () => {
    ...
    try {
      ...
      if (res?.data?.statusCode === 201) {
        ...
        setLoading(false);
        showError("Registration Failed", getErrorMessage(res, "Registration failed. Please try again."));
      }
    } catch (err) {
  ...
  const handleVerifySubmit = async () => {
    ...
    try {
      ...
      if (res?.data?.statusCode === 200) {
        ...
        setLoading(false);
        showError("Verification Failed", getErrorMessage(res, "Invalid or expired code."));
      }
    } catch (err) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (res?.data?.statusCode === 200) {
        ...
        setLoading(false);
        showError("Login Failed", getErrorMessage(res, "Invalid username or password."));
      }
    } catch (err) {
  ...
  const handleForgotSubmit = async () => {
    ...
    try {
      ...
      if (res?.data?.statusCode === 200) {
        ...
        setLoading(false);
        showError("Request Failed", getErrorMessage(res, "Failed to send code. Please try again."));
      }
    } catch (err) {
      console.error("ForgetPassword error:", err);
      setLoading(false);
      showError("Request Failed", "Something went wrong. Please try again.");
    }
  };
  ...
  const handleResetSubmit = async () => {
    ...
    try {
      ...
      if (res?.data?.statusCode === 200) {
        ...
        setLoading(false);
        showError("Reset Failed", getErrorMessage(res, "Failed to reset password. Please try again."));
      }
    } catch (err) {
      console.error("ResetPassword error:", err);
      setLoading(false);
      showError("Reset Failed", "Something went wrong. Please try again.");
    }
  };
      ...
      {view === "signin" && (
        <div className="space-y-4">
          ...
          <div className="text-left">
            <button
              type="button"
              className="text-sm text-red-500 hover:text-red-600 bg-transparent border-none cursor-pointer p-0"
              onClick={() => {
      ...
      {view === "verify" && (
        <div className="space-y-4">
          <div className="flex justify-between gap-2">
            {otp.map((digit, i) => (
              <input
                ...
                className="w-10 h-12 text-center text-lg rounded-md border border-gray-400 text-black focus:border-red-500 focus:outline-none"
              />
            ))}
          </div>
          <div className="flex justify-between items-center  mt-6">
            <button
              type="button"
              className="text-white/80 border  border-white hover:text-white bg-transparent  cursor-pointer p-[9px] mr-2 rounded-lg"
              onClick={() => setView(verifyPurpose === "signup" ? "role" : "forgot")}
            >
              Back
            </button>

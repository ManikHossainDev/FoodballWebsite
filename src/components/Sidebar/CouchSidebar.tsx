...
const CouchSidebar = ({ drawerOpen = false, onCloseDrawer }: CouchSidebarProps) => {
  ...
  const menuList = [
    {
      ...
      icon: <MdOutlineDashboard size={20} className="text-red-400" />,
      href: "/Couch",
    },
    {
      title: "Review Videos",
    ...
    },
    {
      ...
      icon: <PiHeadsetFill size={20} />,
      href: "/BookedConsultations",
    },
    {
      title: "Requests",
  ...
  const MenuItem = ({ item }: any) => {
    ...
    return (
      <Link href={item.href} onClick={closeDrawer}>
        <div
          className={`flex items-center gap-3 px-1 md:px-2 py-3 rounded-xl cursor-pointer transition-all ${
            isActive

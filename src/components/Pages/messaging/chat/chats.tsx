...
const LIMIT_STEP = 10; // proti scroll-e limit koto barbe
const SCROLL_BOTTOM_THRESHOLD = 80; // px — bottom theke koto kache gele "load more" trigger hobe
...
const Chats = ({ receiverId }: ChatsProps) => {
  ...
  const isFetchingMoreRef = useRef<boolean>(false); // ✅ ADDED: guard - eta "load more" response naki prothom load
  ...
  useEffect(() => {
    ...
    // ✅ ADDED: fresh mount/reconnect hole pagination reset
    ...
    const handleConversationsList = (response: IConversationListResponse) => {
      try {
        ...
        // ✅ ADDED: jodi eta "load more" (scroll kore purono conversation chawa) response hoy,
        // tahole notun list-ke niche (bottom-e) append korte hobe, replace na kore.
        if (isFetchingMoreRef.current) {
          setConversations((prev) => {
            const existingIds = new Set(prev.map((c) => c._id).filter(Boolean));
            const uniqueNew = list.filter((c) => !existingIds.has(c._id));

            // server theke ekta o notun conversation na ashle, mane ar purono kichu nai
          ...
          // prothombar load — normal replace
          setConversations(list);
          if (list.length < INITIAL_LIMIT) {
            setHasMore(false);
          }
        }
  ...
  // ✅ ADDED: list-er niche (bottom) scroll korle purono conversation load koro — page & limit dynamically barbe
  ...
  }, [socket, connected, currentUserId, page, limit, loadingMore, hasMore]);
        ...
        >
          {conversations && conversations?.length > 0 ? (
            <>
              ...
              {/* ✅ ADDED: bottom e "load more" indicator */}
              {loadingMore && (
                <div className="w-full flex justify-center py-3">
                  <span className="text-xs text-gray-400">Loading more conversations...</span>
                </div>
              )}
            </>
          ) : (

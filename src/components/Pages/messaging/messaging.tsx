...
// ✅ ADDED: pagination config — scroll korle page o limit dutai barbe
const INITIAL_PAGE = 1;
const INITIAL_LIMIT = 20;
const LIMIT_STEP = 20; // proti scroll-e limit koto barbe
const SCROLL_TOP_THRESHOLD = 80; // px — top theke koto kache gele "load more" trigger hobe
...
const Message = ({ receiverId }: { receiverId: string }) => {
  ...
  // ✅ ADDED: dynamic pagination state
  ...
  const scrollContainerRef = useRef<HTMLDivElement | null>(null); // ✅ ADDED: scrollable section ref
  const isFetchingMoreRef = useRef<boolean>(false); // ✅ ADDED: guard for socket response merge logic
  const prevScrollHeightRef = useRef<number>(0); // ✅ ADDED: scroll position save/restore
  ...
  /*** auto scroll to bottom when new message arrives (only jodi purono message load na hoy) ***/
  useEffect(() => {
    // ✅ jodi ekhon "load more" (purono message) fetch cholche, tahole bottom-e scroll korbo na,
    // karon user upore scroll kore purono message dekhte cheyechilo — niche fele dile UX kharap hobe.
  ...
  /*** socket listeners ***/
  useEffect(() => {
    // conversation switch hole purono message list clear kore dao,
    // nahole notun conversation load howar age purono chat-er message
    // ekhon-o screen-e dekha jete pare
    setAllMessage([]);
    setConversationId(null);

    // ✅ ADDED: notun conversation e dhukle pagination reset kore dao
    ...
    if (socket && connected && receiverId && currentUserId) {
      ...
      const handleConversation = (data: any) => {
        ...
        const sortedIncoming = Array.isArray(list)
          ? [...list].sort(
              (a: any, b: any) =>
        ...
        // ✅ ADDED: jodi eta "load more" (scroll kore purono message chawa) response hoy,
        // tahole notun list-ke age (top-e) prepend korte hobe, replace na kore.
        if (isFetchingMoreRef.current) {
          setAllMessage((prev) => {
            const existingIds = new Set(
              prev.map((m: any) => m?._id).filter(Boolean)
            );
            const uniqueNew = sortedIncoming.filter(
              (m: any) => !m?._id || !existingIds.has(m._id)
            );

            // jodi server theke ekta o notun message na ashe, mane ar purono message nai
          ...
          // prothombar (ba conversation switch) load — normal replace
          setAllMessage(sortedIncoming);
          // jodi prothombari-e limit theke kom message ashe, mane r purono kichu nai
          if (sortedIncoming.length < limit) {
            setHasMore(false);
          }
        }
      };

      // FIX: age kono conversationId check chilo na, tai je kono conversation-er
      // "Message" event ashlei eta current chat window-e jog hoye jeto (message leak
      // - onno conversation-er message eikhane dekha jeto). Ekhon newMsg-er
      // conversation id, current open conversationId-er sathe match korei
      // tobe allMessage-e add kora hobe.
      const handleNewMessage = (data: any) => {
        const newMsg = data?.data?.message ?? data?.data ?? data;
        if (!newMsg) return;

        // newMsg backend theke je format e ashe, tar moddhe conversation id
        // kono ekta field-e thakte pare - shob possible shape check kora hocche
        ...
        // Jodi backend message-er sathe conversation id pathay ebong seta
        // current open conversationId-er sathe na mele, tahole eita onno
        // conversation-er message - eikhane dekhabo na, skip kore dao.
        if (msgConversationId && conversationId && msgConversationId !== conversationId) {
          return;
        }
  ...
  /*** ✅ ADDED: scroll upore gele (top e pouchle) purono message load koro — page & limit dynamically barbe ***/
  const handleScroll = useCallback(() => {
    const container = scrollContainerRef.current;
    if (!container) return;
    if (!socket || !connected || !conversationId || !currentUserId) return;
    if (loadingMore || !hasMore) return;

    if (container.scrollTop <= SCROLL_TOP_THRESHOLD) {
      // scroll position save kore rakhchi jate purono message prepend howar por
      // user er view jump na kore (age jekhane chilo shekhane e thake)
  ...
  }, [socket, connected, conversationId, currentUserId, receiverId, page, limit, loadingMore, hasMore]);

  /*** ✅ ADDED: purono message prepend howar por scroll position restore koro + loading flag off ***/
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container) return;
    if (!isFetchingMoreRef.current) return;

    // notun scrollHeight theke purono scrollHeight bad diye, oi porimane scrollTop set kore dao
  ...
  return (
    <section className="w-full h-full flex flex-col rounded-md  border border-gray-800 ">
      ...
      {/*** Chat Section ***/}
      <section
        ref={scrollContainerRef} // ✅ ADDED
        onScroll={handleScroll} // ✅ ADDED
        className="flex-1 min-h-0 overflow-x-hidden overflow-y-scroll scrollbar scroll-hide relative  bg-opacity-50 px-2 pb-3"
      >
        {/* ✅ ADDED: loading more indicator (top e) */}
        {loadingMore && (
